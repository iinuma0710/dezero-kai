import numpy as np
from dzrkai import cuda, Function
from dzrkai.utils import pair, get_conv_outsize,get_deconv_outsize
...
class Conv2d(Function):
    ...
    def backward(self, gy):
        ...
        gx = deconv2d(gy, W, stride=self.stride, pad=self.pad, outsize=outsize)
...
class Deconv2d(Function):
    ...
    def forward(self, x, W, b):
        ...
        y = col2im(gcol, img_shape, (KH, KW), self.stride, self.pad, to_matrix=False)
...
class Conv2dGradW(Function):
    ...
    def backward(self, gy):
        ...
        gx = deconv2d(gy, gW, stride=self.stride, pad=self.pad, outsize=x.shape[2:])
...
def im2col(x, kernel_size, stride, pad, to_matrix):
...
class Col2im(Function):
    ...
    def forward(self, x):
        self.input_shape = x.shape
        y = col2im_array(x, self.kernel_size, self.stride, self.pad, self.to_matrix)
        return y
...
def col2im(x, input_shape, kernel_size, stride, pad, to_matrix):
...
class MaxPooling2DGrad(Function):
    ...
    def backward(self, ggx):
        return MaxPooling2DWithIndexes(self.mpool2d)(ggx)
